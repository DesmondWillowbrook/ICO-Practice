...
fn trace_1 (node: Node, out: &[Vec<Node>], small_caves_explored: &mut Vec<bool>) -> u64 {
	let mut ans = 0;
	...
	for o in &out[node] {
		ans += match o {
			Node::Small(_) => if !small_caves_explored[usize::from(*o)] {
				...
				let ret = trace_1 (*o, out, small_caves_explored);
				small_caves_explored[usize::from(*o)] = false;
				ret
			} else {0},
			Node::Large(_) => trace_1 (*o, out, small_caves_explored),
			Node::End => 1,
			Node::Start => 0,
		};
	};
...
pub fn part_1<T: std::io::BufRead> (lines: std::io::Lines<T>) -> u64 {
	...
	trace_1(Node::Start, &edges, &mut vec![false; edges.len()])
}

fn trace_2 (node: Node, out: &[Vec<Node>], small_caves_explored: &mut Vec<u8>, twice_cave_used: bool) -> u64 {
	let mut ans = 0;
	...
	for o in &out[node] {
		ans += match o {
			Node::Small(_) => if small_caves_explored[usize::from(*o)] == 0 {
				small_caves_explored[usize::from(*o)] = 1;
				let ret = trace_2 (*o, out, small_caves_explored, twice_cave_used);
				small_caves_explored[usize::from(*o)] = 0;
				ret
			} else if small_caves_explored[usize::from(*o)] == 1 && !twice_cave_used{
				small_caves_explored[usize::from(*o)] = 2;
				let ret = trace_2 (*o, out, small_caves_explored, true);
				small_caves_explored[usize::from(*o)] = 1;
				ret
			} else {0},
			Node::Large(_) => trace_2 (*o, out, small_caves_explored, twice_cave_used),
			Node::End => 1,
			Node::Start => 0,
		};
	};
...
pub fn part_2<T: std::io::BufRead> (lines: std::io::Lines<T>) -> u64 {
	let mut hs = HashMap::from([("start", Node::Start), ("end", Node::End)]);
	...
	//vec![false; edges.len()] is upper bound on number of small nodes. we could count small nodes separately
	//to preserve memory, but I don't want to riddle this already ugly code with more C++-like writing
	trace_2(Node::Start, &edges, &mut vec![0; edges.len()], false)
}